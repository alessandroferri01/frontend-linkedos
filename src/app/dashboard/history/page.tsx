...
export default function HistoryPage() {
  ...
  const [publishModalPost, setPublishModalPost] = useState<Post | null>(null);
  ...
  const loadPosts = useCallback(async (query: PostsQuery) => {
    ...
    try {
      ...
      // Verify LinkedIn posts are still published
      const publishedIds = data.posts
        .filter((p) => p.publishedToLinkedin && p.linkedinPostUrn)
        .map((p) => p.id);
      if (publishedIds.length > 0) {
        api.linkedin.verifyPosts(publishedIds).then(({ removedIds }) => {
          if (removedIds.length > 0) {
            setPosts((prev) =>
              prev.map((p) =>
                removedIds.includes(p.id)
                  ? { ...p, publishedToLinkedin: false, publishedAt: null, linkedinPostUrn: null }
                  : p,
              ),
            );
          }
  ...
  function handleOpenPublishModal(post: Post) {
  ...
  async function handlePublishToLinkedin(content: string) {
    ...
    try {
      await api.linkedin.publishPost(postId, content);
  ...
  return (
    <div className="min-w-0 space-y-6 overflow-hidden">
      ...
      <LinkedInPublishModal
        isOpen={!!publishModalPost}
        content={publishModalPost?.generatedContent || ''}
        topic={publishModalPost?.topic || ''}
